...
export default function ProposalList(props: ProposalListProps) {
  ...
  const list = useMemo(() => {
      ...
      if (lengthFilterArr != 0) {
        ...
        if (props.searchName) {
          ...
          for (let i = 0; i < lengthDataFilter; i++) {
            if (dataFilter[i].tittle.toLocaleLowerCase().includes(props.searchName.toLocaleLowerCase())) {
    ...
    if (props.searchName) {
      ...
      const lengthProposal = dataProposalList.length;
      for (let i = 0; i < lengthProposal; i++) {
        if (dataProposalList[i].tittle.toLocaleLowerCase().includes(props.searchName.toLocaleLowerCase())) {
        ...
        {(status === FetchingStatus.SUCCESS || status === FetchingStatus.UPDATING) && !activating && (
          <>
            {list.length === 0 ? (
              ...
              <>
                {list.map((item: Poll, index: number) => {
